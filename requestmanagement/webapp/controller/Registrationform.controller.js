sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "com.requestmanagement.requestmanagement.controller.Registrationform",
      {
        onInit: function () {
          ...
          let oRegModel = this.getOwnerComponent().getModel(
            "registration-manage"
          );
        ...
        _onRouteMatchedwithreqsendback: async function (oEvent) {
          ...
          try {
            ...
            const reqInfo = await new Promise((resolve, reject) => {
              oModel.read(`/RequestInfo(${this.REQUEST_NO})`, {
                success: resolve,
                error: reject,
              });
            });
            ...
            const aFilters = [
              new sap.ui.model.Filter(
                ...
                this.responseData.COMPANY_CODE
              ),
              new sap.ui.model.Filter(
                "REQUEST_TYPE",
                sap.ui.model.FilterOperator.EQ,
            ...
            if (
              ...
              MessageBox.error(
                "No field configuration data available for selected company code."
              );
              return;
            }
        ...
        _onRouteMatchedwithoutid: function (oEvent) {
          ...
          oModel.read("/FieldConfig", {
            ...
            success: function (res) {
              ...
              var formDataModel = this.buildFormDataBySectionCategory(
                fieldConfig,
                "registration"
              );
            ...
            error: function (err) {
              ...
              MessageBox.error(
                "Failed to load field configuration: " + err.message
              );
            }.bind(this),
          });
        },
          ...
          Object.keys(sections).forEach(
            function (sectionName) {
              ...
              if (sectionName === "Address") {
                ...
                const oTitleBar = new Toolbar({
                  content: [new Title({ text: sectionName, level: "H3" })],
                });
                ...
                Object.keys(sectionData).forEach(
                  function (fieldKey) {
                    ...
                    switch (field.type.toLowerCase()) {
                      case "file":
                        oField = new sap.ui.unified.FileUploader({
                          value: field.value || "",
                          placeholder: field.placeholder || "Choose a file...",
                          required: field.mandatory,
                          visible: field.visible,
                          fileType: field.fileTypes
                            ? field.fileTypes
                                .split(",")
                                .map((type) => type.trim())
                            : undefined,
                          maximumFileSize: field.maximumFileSize
                            ? parseFloat(field.maximumFileSize)
                            : undefined,
                          change: function (oEvent) {
                            ...
                            if (oFile) {
                              ...
                              reader.onload = function (e) {
                                ...
                                const base64Data = base64String.split(",")[1]; // Extract Base64 part
                                const oModel =
                                  this.getView().getModel("formDataModel");
                                const path = `/Supplier Information/${sectionName}/${fieldKey}/value`;
                                oModel.setProperty(path, base64Data);
                                oUploader.setValue(oFile.name); // Display file name
                              }.bind(this);
        ...
        onValueHelpVendorFrag: function (oInput, sectionName, fieldKey, index) {
          ...
          if (!sectionName || !fieldKey) {
            console.error("Invalid parameters in onValueHelpVendorFrag:", {
              oInput,
              sectionName,
              fieldKey,
              index,
            ...
            MessageBox.error(
              "Invalid form field configuration. Please contact support."
            );
            return;
          }
          ...
          console.log(
            "Stored context in onValueHelpVendorFrag:",
            this._oInputContext
          );
            ...
            Fragment.load({
              ...
              .then(
                function (oDialog) {
                  ...
                  this._oValueHelpDialog1.attachOk(
                    function (oEvent) {
                      console.log(
                        "OK event triggered in ValueHelpDialog1 with context:",
                        this._oInputContext
                      );
                      if (!this._oInputContext) {
                        console.error(
                          "Context is undefined in OK event handler"
                        );
                        MessageBox.error(
                          "Invalid context. Please contact support."
                        );
                        return;
                      }
                      this.onValueHelpOkPressRolesVendor(
                        oEvent,
                        this._oInputContext
                      );
                    }.bind(this)
              ...
              .catch(
                function (oError) {
                  ...
                  MessageBox.error(
                    "Unable to load the vendor type selection. Please try again."
                  );
                }.bind(this)
              );
          } else {
        ...
        onValueHelpSubVendorFrag: function (
          ...
          // Validate input parameters
          if (!sectionName || !fieldKey) {
            console.error("Invalid parameters in onValueHelpSubVendorFrag:", {
              oInput,
              sectionName,
              fieldKey,
              index,
          ...
          this._oInputContext = { sectionName, fieldKey, index };
          console.log(
            "Stored context in onValueHelpSubVendorFrag:",
            this._oInputContext
          );
          ...
          if (!this._oValueHelpDialog) {
            Fragment.load({
              ...
              .then(
                function (oDialog) {
                  ...
                  this._oValueHelpDialog.attachOk(
                    function (oEvent) {
                      console.log(
                        "OK event triggered in ValueHelpDialog with context:",
                        this._oInputContext
                      );
                      if (!this._oInputContext) {
                        console.error(
                          "Context is undefined in OK event handler for SubvendorType"
                        );
                        MessageBox.error(
              ...
              .catch(
                function (oError) {
                  console.error(
                    "Failed to load SubvendorType fragment:",
                    oError
                  );
                  MessageBox.error(
                    "Unable to load the sub-vendor type selection dialog. Please try again."
                  );
                }.bind(this)
              );
          } else {
            ...
            this._oInputContext = { sectionName, fieldKey, index };
            console.log(
              "Reusing SubvendorType dialog with context:",
              this._oInputContext
            );
            this._bindVendorTypeData1(this._oValueHelpDialog);
            this._oValueHelpDialog.open();
                ...
                oTable.bindAggregation("rows", {
                  ...
                  events: {
                    dataReceived: function (oEvent) {
                      if (!oEvent.getParameter("data")) {
                        MessageBox.warning(
                          "No sub-vendor type data available."
                        );
                      }
                      oDialog.update();
                    },
        ...
        onValueHelpOkPressRolesVendor: function (oEvent, oContext) {
          console.log("onValueHelpOkPressRolesVendor called with:", {
            oEvent,
            oContext,
          });
          ...
          if (!oContext) {
            console.error(
              "oContext is undefined in onValueHelpOkPressRolesVendor"
            );
          ...
          if (!sectionName || !fieldKey) {
            console.error("Invalid context properties:", {
              sectionName,
              fieldKey,
              index,
            });
          ...
          if (
            !oSelectedData ||
            !oSelectedData.row ||
            !oSelectedData.row.SPRAS ||
            !oSelectedData.row.TXT30
          ) {
          ...
          if (oModel) {
            ...
            if (sectionName === "Address") {
              if (
                oData["Supplier Information"] &&
                oData["Supplier Information"]["Address"] &&
                ...
                console.log(
                  `Updated model: Supplier Information/Address/${index}/${fieldKey} = ${sDisplayValue}`
                );
              } else {
                console.warn(
                  `Invalid model path for Address: Supplier Information/Address/${index}/${fieldKey}`
                );
                MessageBox.error(
                  "Unable to update vendor type. Invalid form configuration."
                );
              }
            } else {
              if (
                oData["Supplier Information"] &&
                oData["Supplier Information"][sectionName] &&
                oData["Supplier Information"][sectionName][fieldKey]
              ) {
                oData["Supplier Information"][sectionName][fieldKey].value =
                  sDisplayValue;
                console.log(
                  `Updated model: Supplier Information/${sectionName}/${fieldKey} = ${sDisplayValue}`
                );
              } else {
                console.warn(
                  `Invalid model path: Supplier Information/${sectionName}/${fieldKey}`
                );
                MessageBox.error(
                  "Unable to update vendor type. Invalid form configuration."
                );
        ...
        onValueHelpOkPressRoles: function (oEvent, oContext) {
          console.log("onValueHelpOkPressRoles called with:", {
            oEvent,
            oContext,
          });
          ...
          if (
            ...
            !oSelectedData.row ||
            !oSelectedData.row.SPRAS ||
            !oSelectedData.row.TEXT40
          ) {
            console.error(
              "Invalid selection data in onValueHelpOkPressRoles:",
              oSelectedData
            );
            MessageBox.error(
              "Invalid sub-vendor type selection data. Please try again."
            );
            return;
          }
          ...
          if (oModel) {
            ...
            if (sectionName === "Address") {
              if (
                ...
                MessageBox.error(
                  "Unable to update sub-vendor type. Invalid form configuration."
                );
              }
            } else {
              if (
                  ...
                  Object.keys(bank).forEach(
                    function (fieldKey) {
                      ...
                      if (field.visible) {
                            ...
                            if (
                              ...
                              const items = field.dropdownValues
                                ? [
                                    ...
                                    ...field.dropdownValues.split(",").map(
                                      (opt) =>
                                        new sap.ui.core.Item({
                                          key: opt.trim(),
                                          text: opt.trim(),
                        ...
                        console.log(
                          `Added field: ${field.label} in ${bankType}`
                        );
                      }
                    }.bind(this)
                  );
        ...
        createDisclosureForm: function (oDisclosureFields, type) {
          ...
          if (type === "registration" || type === "sendback") {
            if (
              !oDisclosureFields ||
              typeof oDisclosureFields !== "object" ||
              Object.keys(oDisclosureFields).length === 0
            ) {
              console.error(
                "No fields found for Disclosure or invalid object. Check oDisclosureFields:",
                oDisclosureFields
              );
              return;
            }
            ...
            var oContainer = this.getView().byId(
              this.getView().createId("DisclosureFormContainer")
            );
            if (!oContainer) {
              console.error(
                "DisclosureFormContainer not found. Check your view XML."
              );
              return;
            }
            ...
            var fieldMapping = {
              ...
              "Anti-Corruption & Anti Bribery Regulation":
                "anticorruptionregulation",
            ...
            Object.keys(oDisclosureFields).forEach(function (category) {
              ...
              Object.keys(categoryData).forEach(function (fieldKey) {
                ...
                if (field.visible) {
                  ...
                  var valueToSet =
                    existingValue !== undefined ? existingValue : 1;
              ...
              Object.keys(categoryData).forEach(function (fieldKey) {
                ...
                if (field.visible) {
                  ...
                  var oRadioButtonGroup = new RadioButtonGroup({
                    ...
                    select: function (oEvent) {
                      var oDisclosureModel = that
                        .getView()
                        .getModel("disclosureModel");
                      if (oDisclosureModel) {
                        var selectedIndex =
                          oEvent.getParameter("selectedIndex");
                        oDisclosureModel.setProperty(
                          propertyMap[field.label],
                          selectedIndex
                        );
                        console.log(
                          "Radio button changed for",
                          field.label,
                          "property:",
                          propertyMap[field.label],
                          "new value:",
                        ...
                        console.error(
                          "disclosureModel not available during select event."
                        );
                      }
                    }.bind(this),
                  });
        ...
        createAttachmentsForm: function (
          oAttachmentFields,
          ...
          if (this._bAttachmentSectionCreated && !bForceRecreate) {
            console.log(
              "Attachment section already created, skipping re-render"
            );
            return;
          }
          ...
          if (!oContainer) {
            console.error(
              "AttachmentsFormContainer not found. Check your view XML."
            );
            return;
          }
          ...
          if (!oFormDataModel) {
            console.error(
              "formDataModel not found in the view. Ensure it is set in onInit."
            );
            return;
          }
          ...
          var transformedFields = oAttachmentFields.map((field) => {
            const updatedField = {
              ...
              fieldPath:
                field.fieldPath ||
                (field.title || "DEFAULT").toUpperCase().replace(/\s+/g, "_"),
          ...
          if (!oFormDataModel.getProperty("/Attachments") || bForceRecreate) {
            ...
            console.log(
              "Model after setting Attachments:",
              oFormDataModel.getProperty("/Attachments")
            );
          }
          ...
          this.openDocumentDialog = function () {
            ...
            if (
              !this._oDialog ||
              !(this._oDialog instanceof sap.m.Dialog) ||
              this._oDialog.bIsDestroyed
            ) {
              try {
                ...
                if (!(this._oDialog instanceof sap.m.Dialog)) {
                  console.error(
                    "Fragment did not return a Dialog object:",
                    this._oDialog
                  );
                  sap.m.MessageToast.show(
                    "Failed to load document dialog. Please check the fragment configuration."
                  );
                  return;
                }
                ...
                sap.m.MessageToast.show(
                  "Error loading document dialog. Please contact support."
                );
                return;
              }
            }
          ...
          this.onSubmitDocumentName = function () {
            ...
            var bTitleExists = oFormDataModel
              .getProperty("/Attachments")
              .some(
                (field) => field.title.toLowerCase() === sNewTitle.toLowerCase()
              );
            if (bTitleExists) {
            ...
            if (!oExistingVBox) {
                  ...
                  new Button({
                    ...
                    press: function (oEvent) {
                      ...
                      if (oContext) {
                        ...
                        oFileUploader.setBindingContext(
                          oContext,
                          "formDataModel"
                        );
                        oContainer.addContent(oFileUploader);
                        setTimeout(() => {
                          var $fileInput = oFileUploader
                            .$()
                          ...
                          if ($fileInput.length > 0) {
                            ...
                            if (!oFileUploader.getFocusDomRef()) {
                              console.warn(
                                "FileUploader focus issue detected after delay"
                              );
                            }
                          } else {
                            console.error(
                              "File input not found in FileUploader"
                            );
                          }
                        ...
                        console.error(
                          "No binding context found for Upload button"
                        );
                      }
                    },
                  }),
                    ...
                    items: [
                      new Button({
                        ...
                        press: function (oEvent) {
                          var oContext = oEvent
                            .getSource()
                            .getBindingContext("formDataModel");
                          var sBase64Data = oContext?.getProperty("imageUrl");

                          if (
                            !sBase64Data ||
                            typeof sBase64Data !== "string" ||
                            !sBase64Data.startsWith("data:")
                          ) {
                            sap.m.MessageToast.show(
                              "No valid file available to open."
                            );
                            return;
                          }
              ...
              if (oNewTableBinding) {
                ...
                console.warn(
                  `Binding for ${sNewTitle} table not found, attempting delayed refresh`
                );
                setTimeout(() => {
                  ...
                  if (delayedBinding) {
                    ...
                    console.log(
                      `Delayed refresh successful for ${sNewTitle} table`
                    );
                  } else {
                    console.error(
                      `Delayed binding for ${sNewTitle} table still not found`
                    );
                  }
                }, 100);
              }
              ...
              if (oExistingTable) {
                ...
                if (oExistingBinding) {
                  ...
                  console.warn(
                    `Binding for existing ${sNewTitle} table not found, attempting delayed refresh`
                  );
                  setTimeout(() => {
                    ...
                    if (delayedBinding) {
                      delayedBinding.refresh();
                      console.log(
                        `Delayed refresh successful for existing ${sNewTitle} table`
                      );
                    } else {
                      console.error(
                        `Delayed binding for existing ${sNewTitle} table still not found`
                      );
                    }
                  }, 100);
                }
          ...
          this.onRemoveDocumentSection = function (sTitle) {
            // Confirm deletion
            MessageBox.confirm(
              `Are you sure you want to remove the "${sTitle}" section?`,
              {
                ...
                onClose: function (oAction) {
                  if (oAction === MessageBox.Action.OK) {
                    ...
                    var oAttachments =
                      oFormDataModel.getProperty("/Attachments");
                    oAttachments = oAttachments.filter(
                      (field) => field.title !== sTitle
                    );
                    oFormDataModel.setProperty("/Attachments", oAttachments);
                    oFormDataModel.refresh(true);
                    ...
                    var oVBoxToRemove = this._oMainVBox
                      .getItems()
                      .find((item) => {
                        return (
                          item instanceof VBox &&
                    ...
                    sap.m.MessageToast.show(
                      `Section "${sTitle}" removed successfully`
                    );
                  }
                }.bind(this),
              }
          ...
          if (bForceRecreate || !this._bAttachmentSectionCreated) {
            ...
            setTimeout(() => {
              aTables.forEach(({ table, title }) => {
                ...
                if (oBinding) {
                  oBinding.refresh();
                  console.log(
                    `Delayed refresh successful for table with title: ${title}`
                  );
                  console.log(
                    `Binding contexts for ${title}:`,
                    oBinding.getContexts()
                  );
                } else {
                  console.error(
                    `Delayed binding for table with title ${title} still not found.`
                  );
                }
              });
            }, 100);
            ...
            var sCssPath = sap.ui.require.toUrl(
              "com/aispsuppform/aispsupplierform/css/AttachmentsForm.css"
            );
          ...
          console.log(
            "Final Attachments in model after setup:",
            oFormDataModel.getProperty("/Attachments")
          );
        },
        ...
        onDescriptionChange: function (oEvent) {
          ...
          console.log(
            `Description changed to: ${sNewDescription} at path: ${sPath}`
          );
        ...
        createQualityCertificatesForm: function (qualityCertificatesData) {
          if (
            !qualityCertificatesData ||
            typeof qualityCertificatesData !== "object" ||
            Object.keys(qualityCertificatesData).length === 0
          ) {
            console.error(
              "No fields found for Quality Certificates or invalid object. Check qualityCertificatesData:",
              qualityCertificatesData
            );
            return;
          }
          ...
          var oContainer = this.getView().byId(
            this.getView().createId("QualityCertificatesFormContainer")
          );
          if (!oContainer) {
            console.error(
              "QualityCertificatesFormContainer not found. Check your view XML."
            );
            return;
          }
                ...
                if (category === "Custom Certifications") {
                  oRow.addCell(
                    new Button({
                      ...
                      press: this._onRemoveCertificate.bind(
                        this,
                        category,
                        fieldKey
                      ),
        ...
        _openAddCertificateFragment: function () {
          ...
          sap.ui
            .getCore()
            .byId(this.getView().createId("certificateNameInput"))
            .setValue("");
          this._oAddCertificateDialog.open();
        },

        _onAddCertificateSubmit: function () {
          var sCertificateName = sap.ui
            .getCore()
            .byId(this.getView().createId("certificateNameInput"))
            .getValue();
          if (!sCertificateName) {
          ...
          var oTable = this.getView()
            .byId(this.getView().createId("QualityCertificatesFormContainer"))
            .getContent()[0];
        ...
        _onRemoveCertificate: function (sCategory, sFieldKey, oEvent) {
          ...
          if (
            ...
            if (
              Object.keys(oData["Quality Certificates"][sCategory]).length === 0
            ) {
        ...
        handleUploadComplete: function (oEvent) {
          ...
          if (sResponse) {
            ...
            if (oContext) {
              ...
              var oFormDataModel = this.getView().getModel("formDataModel");
              oFormDataModel.setProperty(
                oContext.getPath() + "/fileName",
                sFileName
              );
              oFormDataModel.setProperty(
                oContext.getPath() + "/uploaded",
                true
              );
            ...
            console.warn(
              "No response in uploadComplete, relying on change event"
            );
          }
        },
          ...
          if (oContext) {
            ...
            if (oFile) {
              ...
              reader.onload = function (e) {
                ...
                console.log(
                  "Change event: File converted to base64:",
                  oFile.name,
                  "imageUrl:",
                  imageUrl
                );
              ...
              console.log(
                "Value changed, fileName updated:",
                oFileUploader.getValue()
              );
            }
          }
        ...
        handleTypeMissmatch: function (oEvent) {
          ...
          MessageBox.error(
            "The file type *." +
              ...
              " is not supported. Please upload only *." +
              aFileTypes +
              " files."
          );
        },
        ...
        buildPayload: function (actionType) {
          ...
          var oDisclosureModel = this.getView()
            .getModel("disclosureModel")
            .getData();
          ...
          const collectDynamicFieldsFromCategory = (
            categoryData,
            dynamicFields
          ) => {
            Object.keys(categoryData).forEach((fieldKey) => {
              ...
              if (
                field &&
                typeof field === "object" &&
                (field.newDynamicFormField === true ||
                  field.newDynamicFormField === "yes")
              ) {
                dynamicFields[fieldKey] = field.value || "";
                console.log(
                  `Collected dynamic field: ${fieldKey} = ${field.value}`,
                  field
                );
              }
            });
          ...
          Object.keys(oFormData).forEach((sectionName) => {
            ...
            Object.keys(sectionData).forEach((categoryName) => {
              ...
              console.log(
                `Processing category: ${sectionName}/${categoryName}`
              );
              ...
              if (Array.isArray(categoryData)) {
                  ...
                  if (Object.keys(dynamicFields).length > 0) {
                    ...
                    if (categoryName === "Address") {
                      ...
                      customCategory =
                        index === 0
                          ? "Primary Bank"
                          : item.BANK_TYPE || "Other Bank Details";
                    }
                    ...
                    console.log(
                      `Added dynamic fields for ${sectionName}/${customCategory}`,
                      dynamicFields
                    );
                  }
                });
                ...
                if (Object.keys(dynamicFields).length > 0) {
                  ...
                  console.log(
                    `Added dynamic fields for ${sectionName}/${categoryName}`,
                    dynamicFields
                  );
                }
              }
          ...
          console.log(
            "DynamicFormFields:",
            JSON.stringify(dynamicFormFields, null, 2)
          );

          // Debug Disclosure_Fields
          console.log(
            "disclosureModel:",
            JSON.stringify(oDisclosureModel, null, 2)
          );
          ...
          var payload = {
            ...
            contactsData: [
              {
                ...
                CITY:
                  oFormData["Supplier Information"]["Primary Contact"]["CITY"]
                    ?.value || "",
                STATE:
                  oFormData["Supplier Information"]["Primary Contact"]["STATE"]
                    ?.value || "",
                COUNTRY:
                  oFormData["Supplier Information"]["Primary Contact"][
                    "COUNTRY"
                ...
                EMAIL:
                  oFormData["Supplier Information"]["Primary Contact"]["EMAIL"]
                    ?.value || "",
                CONTACT_NO:
                  oFormData["Supplier Information"]["Primary Contact"][
                    "CONTACT_NUMBER"
                ...
                MOBILE_NO:
                  oFormData["Supplier Information"]["Primary Contact"]["MOBILE"]
                    ?.value || "",
              },
            ],
            DyanamicFormFields: dynamicFormFields,
            bankData: oFormData["Finance Information"][
              ...
              GST:
                oFormData["Finance Information"]["TAX-VAT-GST"].GST_NO?.value ||
                "",
              GSTYES_NO:
                oFormData["Finance Information"]["TAX-VAT-GST"]["TAX/VAT/GST"]
                  ?.value || "",
            })),
            ...
            Quality_Certificates: Object.keys(
              ...
              .map((key) => {
                ...
                return {
                  ...
                  AVAILABLE: oFormData["Quality Certificates"][
                    "Standard Certifications"
                  ][key].value
                    ? "YES"
                    : "NO",
                  DONE_BY:
                    oFormData["Quality Certificates"][
                      "Standard Certifications"
                    ][key].value || "",
                };
              }),
        ...
        onAddOtherOfficeAddress: function () {
          ...
          var primaryAddress = formData["Supplier Information"]["Address"].find(
            (addr) => addr.ADDRESS_TYPE === "Primary"
          );
          ...
          console.log(
            "Updated formDataModel with Other Office Address:",
            formData
          );
        ...
        onAddOtherBankDetails: function () {
          var oFormDataModel = this.getView().getModel("formDataModel");
          var formData = oFormDataModel.getData();
          var primaryBank = formData["Finance Information"][
            "Primary Bank details"
          ].find((b) => b.BANK_TYPE === "Primary");
          var otherBank = JSON.parse(JSON.stringify(primaryBank));
          otherBank.BANK_TYPE = "Other Bank Details";
          formData["Finance Information"]["Primary Bank details"].push(
            otherBank
          );
          oFormDataModel.setData(formData);
          console.log(
            "Updated formDataModel with Other Bank Details:",
            formData
          );
          this.createDynamicForm(formData);
