sap.ui.define([
    ...
    function (Controller, UIComponent, JSONModel, MessageBox, Fragment, Filter, FilterOperator, MessageToast, HTML, SimpleForm, Label, Input, Button, RadioButton, RadioButtonGroup, Toolbar, Title, ToolbarSpacer, VBox) {
        "use strict";

        return Controller.extend("com.reqreportsaisp.reqreportsaisp.controller.RequestDetails", {
            onInit: function () {
                ...
                oRouter.getRoute("ReportObj").attachPatternMatched(this.onObjectMatched, this);
            },
            ...
            getFormValues: function (req, cCode) {
                ...
                // Validate inputs
                if (!req || !cCode) {
                    sap.m.MessageToast.show("Request number or company code is missing");
                    oView.setBusy(false);
                    return;
                }
                ...
                const aFilters = [
                    new sap.ui.model.Filter("COMPANY_CODE", sap.ui.model.FilterOperator.EQ, cCode),
                    new sap.ui.model.Filter("REQUEST_TYPE", sap.ui.model.FilterOperator.EQ, req)
                ];
                ...
                oModel.read("/FieldConfig", {
                    ...
                    success: function (fieldCfgRes) {
                        let fieldConfig = fieldCfgRes.results || fieldCfgRes.value || fieldCfgRes;
                        if (!fieldConfig || (Array.isArray(fieldConfig) && fieldConfig.length === 0)) {
                            sap.m.MessageToast.show("No field configuration data available for the provided request number and company code.");
                        ...
                        const oFormDataModel = new sap.ui.model.json.JSONModel(formDataModel);
            ...
            createSupplierFormPage: function (supplierData, pageType, dynamicFields) {
                ...
                if (!oRequestInfoModel) {
                    oRequestInfoModel = new sap.ui.model.json.JSONModel({ editable: pageType === "registration" });
                ...
                dynamicData.forEach(function (dynamicField) {
                    ...
                    if (SECTION === "Supplier Information" && CATEGORY.startsWith("PrimaryAddress")) {
                        ...
                        if (!oModel.getProperty(`/Supplier Information/Address/${addressIndex}`)) {
                            oModel.getProperty("/Supplier Information/Address").push({});
                        }
                        Object.keys(parsedData).forEach(fieldKey => {
                            oModel.setProperty(`/Supplier Information/Address/${addressIndex}/${fieldKey}`, {
                    ...
                    } else if (SECTION === "Supplier Information" && CATEGORY === "Other Office Address") {
                        let addressIndex = oModel.getProperty("/Supplier Information/Address")?.length || 1;
                        if (!oModel.getProperty("/Supplier Information/Address")) {
                            oModel.setProperty("/Supplier Information/Address", []);
                        }
                        if (!oModel.getProperty(`/Supplier Information/Address/${addressIndex}`)) {
                ...
                var oHorizontalLine = new sap.ui.core.HTML({
                    content: "<hr style='border: 1px solid #d9d9d9; width: 100%; margin: 10px 0;' />"
                });
                
                // Calculate maximum label length for uniform width
                let maxLabelLength = 0;
                Object.keys(sections).forEach(function (sectionName) {
                    const sectionData = oModel.getProperty(`/Supplier Information/${sectionName}`) || sections[sectionName];
                ...
                Object.keys(sections).forEach(function (sectionName) {
                    ...
                    if (sectionName === "Address") {
                        sectionData.forEach(function (address, index) {
                            const addressType = address.ADDRESS_TYPE || (index === 0 ? "Primary" : "Other Office Address");
                            ...
                            oSectionVBox.addItem(new sap.m.Title({
            ...
            createFinanceFormPage: function (financeData) {
                ...
                if (financeData["TAX-VAT-GST"]) {
                    Object.keys(financeData["TAX-VAT-GST"]).forEach(function (fieldKey) {
                ...
                Object.keys(categories).forEach(function (categoryName) {
                    ...
                    categoryData.forEach(function (bank, index) {
                        ...
                        oFormVBox.addItem(new sap.m.Title({
                        ...
                        Object.keys(bank).forEach(function (fieldKey) {
                            ...
                            if (field.visible) {
                                ...
                                var oText = new sap.m.Text({
                                    text: "{formDataModel>/Finance Information/Primary Bank details/" + index + "/" + fieldKey + "/value}",
                                    width: "20rem",
                                    visible: "{= !${requestInfo>/editable} }",
                                    customData: [
                                        new sap.ui.core.CustomData({
                                ...
                                var oInput = new sap.m.Input({
                                    value: "{formDataModel>/Finance Information/Primary Bank details/" + index + "/" + fieldKey + "/value}",
                                    width: "20rem",
                                    visible: "{requestInfo>/editable}",
                                ...
                                aFields.push(oFieldHBox);
                                console.log(`Added field: ${field.label} in ${bankType} with value: ${field.value}`);
                            }
                        }.bind(this));
                ...
                if (taxData) {
                    ...
                    // Add title with H6 size
                    oTaxFormVBox.addItem(new sap.m.Title({
                        text: "TAX/VAT/GST",
                        level: "H6",
                        titleStyle: "H6"
                    }));
                    ...
                    Object.keys(taxData).forEach(function (fieldKey) {
                        ...
                        if (field.visible) {
                            ...
                            var oText = new sap.m.Text({
                                text: "{formDataModel>/Finance Information/TAX-VAT-GST/" + fieldKey + "/value}",
                                width: "20rem",
                                visible: "{= !${requestInfo>/editable} }",
                                customData: [
                                    new sap.ui.core.CustomData({
                            ...
                            var oInput = new sap.m.Input({
                                value: "{formDataModel>/Finance Information/TAX-VAT-GST/" + fieldKey + "/value}",
                                width: "20rem",
                                visible: "{requestInfo>/editable}",
                                change: this.onFieldChange.bind(this),
                                customData: [
                            ...
                            console.log(`Added field: ${field.label} in TAX-VAT-GST with value: ${field.value}`);
                        }
                    }.bind(this));
            ...
            createOperationalFormPage: function (operationalData) {
                ...
                Object.keys(categories).forEach(function (categoryName, index, array) {
                    var categoryData = categories[categoryName];
                    if (!categoryData) {
                        console.warn(`No data for category: ${categoryName}`);
                        return;
            ...
            createDisclosureForm: function (disclosureData, type) {
                ...
                if (!oContainer) {
                    console.error("DisclosureFormContainer not found in view:", this.getView().getId());
                    sap.m.MessageBox.error("Disclosure container not found. Check view XML for ID 'DisclosureFormContainer'.");
                    return;
                }
                oContainer.removeAllContent();
                oContainer.setWidth("100%");
                console.log("Container cleared and width set to 100%:", oContainer.getId());
                console.log("Container type:", oContainer.getMetadata().getName());
                
                // Log raw disclosureData
                console.log("Raw disclosureData:", JSON.stringify(disclosureData, null, 2));
                if (!disclosureData || Object.keys(disclosureData).length === 0) {
                    console.warn("disclosureData is empty or invalid");
                    oContainer.addContent(new sap.m.Text({ text: "No disclosure data available" }));
                    return;
                }
                ...
                if (!oRequestInfoModel) {
                    ...
                    oRequestInfoModel = new sap.ui.model.json.JSONModel({ editable: type === "registration" });
                    this.getView().setModel(oRequestInfoModel, "requestInfo");
                }
                ...
                if (!oModel.getProperty("/Disclosures")) {
                    ...
                    Object.keys(disclosureData).forEach(function (categoryName) {
                        ...
                        Object.keys(disclosureData[categoryName]).forEach(function (fieldKey) {
                            var field = disclosureData[categoryName][fieldKey];
                            console.log(`disclosureData value for ${categoryName}/${fieldKey}:`, field.value);
                    ...
                    console.log("Disclosures already exists in formDataModel:", JSON.stringify(oModel.getProperty("/Disclosures"), null, 2));
                }
                
                // Log model data
                console.log("formDataModel data after initialization:", JSON.stringify(oModel.getData(), null, 2));
                ...
                Object.keys(disclosureData).forEach(function (categoryName) {
                    ...
                    Object.keys(categoryData).forEach(function (fieldKey) {
                        const field = categoryData[fieldKey];
                        if (field.visible) {
                            maxLabelLength = Math.max(maxLabelLength, (field.label || fieldKey).length);
                        }
                    });
                });
                // Convert max label length to width (approximate 0.5rem per character)
                ...
                Object.keys(disclosureData).forEach(function (categoryName) {
                    ...
                    console.log(`Processing category: ${categoryName}`, JSON.stringify(categoryData, null, 2));
                    ...
                    Object.keys(categoryData).forEach(function (fieldKey) {
                        var field = categoryData[fieldKey];
                        if (!field.visible) {
                            console.log(`Skipping ${categoryName}/${fieldKey} as not visible`);
                            return;
                        }
                        
                        var sBindingPath = `/Disclosures/${categoryName}/${fieldKey}/value`;
                        var fieldValue = oModel.getProperty(sBindingPath);
                        console.log(`Field value for ${categoryName}/${fieldKey}:`, fieldValue);
                        ...
                        var oControl = new sap.m.Select({
                            ...
                            change: function (oEvent) {
                                ...
                                console.log(`Updated value for ${sBindingPath}:`, selectedKey);
                            }.bind(this)
                        });
                        ...
                        aFields.push(oFieldHBox);
                        console.log(`Added field: ${field.label || fieldKey} in ${categoryName}`);
                        bHasFields = true;
                    }.bind(this));
                ...
                if (!bHasFields) {
                    ...
                    oFlexBox.addItem(new sap.m.Text({ text: "No disclosure fields available" }));
                }
            },
            ...
            createSubmission: function (subData) {
                ...
                if (!oModel.getProperty("/Submission")) {
                    ...
                    Object.keys(subData).forEach(function (categoryName) {
                        ...
                        Object.keys(subData[categoryName]).forEach(function (fieldKey) {
                            ...
                            console.log(`subData value for ${categoryName}/${fieldKey}:`, field.value);
                            oFormData.Submission[categoryName][fieldKey] = {
                                value: field.value || (field.type.toLowerCase() === "checkbox" ? false : "") // Fallback for checkbox or text
                            };
                        });
                    });
                    oModel.setData(oFormData, true); // Merge with existing data
                } else {
                    console.log("Submission already exists in formDataModel:", oModel.getProperty("/Submission"));
                }
                ...
                for (var i = 0; i < aInputFields.length; i += 3) {
                    ...
                    var firstColumn = firstField ? this._createFieldVBox(firstField.fieldKey, firstField.field, "Submission/Declaration") : new sap.m.VBox({ width: "100%" });
                    var secondColumn = secondField ? this._createFieldVBox(secondField.fieldKey, secondField.field, "Submission/Declaration") : new sap.m.VBox({ width: "100%" });
                    var thirdColumn = thirdField ? this._createFieldVBox(thirdField.fieldKey, thirdField.field, "Submission/Declaration") : new sap.m.VBox({ width: "100%" });
                ...
                aCheckboxes.forEach(function (checkbox) {
                    var oCheckboxVBox = this._createFieldVBox(checkbox.fieldKey, checkbox.field, "Submission/Declaration");
            ...
            _createFieldVBox: function (fieldKey, field, sectionPath) {
                ...
                var oModel = this.getView().getModel("formDataModel");
                var fieldValue = oModel.getProperty(sBindingPath);
                console.log(`Field value for ${sectionPath}/${fieldKey}:`, fieldValue);

                // Non-editable Text control for display mode (for text fields)
                var oText = field.type.toLowerCase() !== "checkbox" ? new sap.m.Text({
                    text: "{formDataModel>" + sBindingPath + "}",
                    width: "20rem",
                    visible: "{= !${requestInfo>/editable} }"
                }) : null;
                ...
                switch (field.type.toLowerCase()) {
                    case "text":
                        oControl = new sap.m.Input({
                            ...
                            type: field.label.toLowerCase().includes("email") ? "Email" :
                                field.label.toLowerCase().includes("contact") ? "Tel" :
                                    field.label.toLowerCase().includes("date") ? "Date" : "Text",
                            required: field.mandatory,
                            visible: "{requestInfo>/editable}",
                            minLength: field.minimum ? parseInt(field.minimum) : undefined,
                            maxLength: field.maximum ? parseInt(field.maximum) : undefined,
                ...
                var items = field.type.toLowerCase() === "checkbox" ? [oLabel, oControl] : [oLabel, oText, oControl];
            ...
            onFieldChange: function (oEvent) {
                ...
                console.log(`Updated formDataModel at ${sPath} with value: ${sValue}`);
            },
            ...
            _submitPostRegAction: function (actionType, sComment) {
                ...
                const oRequestInfoData = oRequestInfoModel ? oRequestInfoModel.getData() : {};
                ...
                if (!oFormData) {
                    ...
                    sap.m.MessageBox.error("Form data is not loaded. Please reload the form and try again.");
                    return;
                }
                ...
                const extractCustomFields = (section, category, data, srNo = 0) => {
                    ...
                    Object.keys(data).forEach(key => {
                        if (key !== "ADDRESS_TYPE" && key !== "BANK_TYPE" && data[key]?.newDynamicFormField) {
                ...
                Object.keys(oFormData).forEach(section => {
                    ...
                    if (section === "Disclosures") {
                        Object.keys(sectionData).forEach(category => {
                            extractCustomFields(section, "Disclosures", sectionData[category]);
                        });
                    } else {
                        Object.keys(sectionData).forEach(category => {
                            ...
                            if (Array.isArray(categoryData)) {
                                categoryData.forEach((record, index) => {
                                    ...
                                    if (section === "Supplier Information" && category === "Address") {
                                        dynamicCategory = record.ADDRESS_TYPE === "Primary" ? "PrimaryAddress" : "Other Office Address";
                                    } else if (section === "Finance Information" && category === "Primary Bank details") {
                                        dynamicCategory = record.BANK_TYPE === "Primary" ? "Primary Bank" : "Other Bank Details";
                                    }
                ...
                const payload = {
                    ...
                    reqHeader: [{
                        REGISTERED_ID: oFormData["Supplier Information"]?.["Supplier Information"]?.REGISTERED_ID?.value || oRequestInfoData.REGISTERED_ID || "",
                        WEBSITE: oFormData["Supplier Information"]?.["Supplier Information"]?.WEBSITE?.value || oRequestInfoData.WEBSITE || "",
                        VENDOR_NAME1: oFormData["Supplier Information"]?.["Supplier Information"]?.VENDOR_NAME1?.value || oRequestInfoData.VENDOR_NAME1 || "",
                        COMPLETED_BY: oFormData["Submission"]?.["Declaration"]?.COMPLETED_BY?.value || oRequestInfoData.COMPLETED_BY || "",
                        DESIGNATION: oFormData["Submission"]?.["Declaration"]?.DESIGNATION?.value || oRequestInfoData.DESIGNATION || "",
                        SUBMISSION_DATE: oFormData["Submission"]?.["Declaration"]?.SUBMISSION_DATE?.value || oRequestInfoData.SUBMISSION_DATE || "",
                        COMPANY_CODE: oFormData["Supplier Information"]?.["Supplier Information"]?.COMPANY_CODE?.value || oRequestInfoData.COMPANY_CODE || "",
                        REQUEST_TYPE: oRequestInfoData.REQUEST_TYPE || ""
                    }],
            
                    addressData: (oFormData["Supplier Information"]?.Address || []).map((addr, index) => ({
                        SR_NO: index,
                    ...
                    contactsData: [oFormData["Supplier Information"]?.["Primary Contact"] || {}].map((c, index) => ({
                        SR_NO: index,
                    ...
                    Operational_Prod_Desc: (Array.isArray(oFormData["Operational Information"]?.["Product-Service Description"])
                        ? oFormData["Operational Information"]["Product-Service Description"]
                        : [oFormData["Operational Information"]?.["Product-Service Description"] || {}]).map((p, index) => ({
                            SR_NO: index,
                    ...
                    Operational_Capacity: (Array.isArray(oFormData["Operational Information"]?.["Operational Capacity"])
                        ? oFormData["Operational Information"]["Operational Capacity"]
                        : [oFormData["Operational Information"]?.["Operational Capacity"] || {}]).map((cap, index) => ({
                            SR_NO: index,
                    ...
                    Disclosure_Fields: oFormData.Disclosures ? [{
                        SR_NO: 0,
                        INTEREST_CONFLICT: oFormData.Disclosures["Conflict of Interest"]?.CONFLICT_OF_INTEREST?.value || "",
                        ANY_LEGAL_CASES: oFormData.Disclosures["Legal Case Disclosure"]?.LEGAL_DISCLOSURE?.value || "",
                        ABAC_REG: oFormData.Disclosures["Anti-Corruption Regulation"]?.ANTI_CORRUPTION?.value || "",
                        CONTROL_REGULATION: oFormData.Disclosures["Export Control"]?.EXPORT_CONTROL?.value || ""
                    }] : [],
            
                    Quality_Certificates: (oRequestInfoData.TO_QA_CERTIFICATES?.results || []).map(q => ({
                        SR_NO: q.SR_NO,
                    ...
                    Attachments: (oRequestInfoData.TO_ATTACHMENTS?.results || []).map(file => ({
                ...
                oODataModel.create("/PostRegData", payload, {
                    ...
                    error: function (oError) {
                        ...
                        sap.m.MessageBox.error(`Failed to ${actionType.toLowerCase()}: ${errorMsg}`);
                    }
                });
            },
