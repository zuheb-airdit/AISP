sap.ui.define(
  ...
  (
    ...
    return Controller.extend(
      "com.usermasteraisp.usermasteraisp.controller.Usermaster",
      {
        formatter: formatter,
        onInit() {
          ...
          this.getIasUsers();
          this.isSubmitting = false; // This is the lock flag
        },
            ...
            if (oCtx1) {
              ...
              if (sCompany && oMultiInput1) {
                ...
                aCode.forEach(function (code, index) {
                  ...
                  console.log(
                    `Added token: ${tokenText} with key: ${trimmedCode}`
                  );
                });
              }
            }
        ...
        onRemainingRolesClick: function (oEvent) {
          let sRoles = oEvent
            .getSource()
            .getBindingContext()
            .getProperty("USER_ROLE");
          let rolesArray = sRoles
            ? sRoles.split(",").map((role) => role.trim())
            : [];
        ...
        onCreateUser: function () {
          ...
          if (!this.createFragment) {
            this.createFragment = sap.ui.xmlfragment(
              "com.usermasteraisp.usermasteraisp.fragments.createUserMaster",
              this
            );
        ...
        // Define this outside of the function (preferably in the controller constructor)

        onSumbitUserMaster: function () {
          let that = this;

          // If the function is already submitting, return early to prevent double submission
          if (this.isSubmitting) {
            return;
          }

          // Lock the function to prevent multiple submissions
          this.isSubmitting = true;

          // Disable the submit button to prevent further clicks
          let submitButton = sap.ui.getCore().byId("idSubmitButton");
          if (submitButton) {
          ...
          // Reset ValueState for all input fields at the start
          ...
          let aSelectedRoles = oUserRoleComboBox
            .getSelectedItems()
            .map((item) => item.getKey())
            .join(", ");
          let aSelectedCompanyCodes = oCompanyComboBox
            .getSelectedItems()
            .map((item) => item.getKey())
            .join(",");
          let aSelectedCompanyDescs = oCompanyComboBox
            .getSelectedItems()
          ...
          if (!userName) {
            ...
            sap.ui
              .getCore()
              .byId("idUserName")
              .setValueStateText("This field is required.");
            bValidationError = true;
          } else {
            sap.ui
              .getCore()
          ...
          if (!aSelectedRoles) {
            ...
            sap.ui
              .getCore()
              .byId("idUserRole")
              .setValueStateText("At least one role must be selected.");
            bValidationError = true;
          } else {
            sap.ui
              .getCore()
          ...
          if (bValidationError) {
            ...
            if (submitButton) {
              submitButton.setEnabled(true); // Re-enable the button if there's a validation error
            }

            // Unlock the submit button and allow further submissions
            ...
            success: function (res) {
              ...
              oModel.create("/CreateVMUser", oPayload, {
                success: function () {
                  ...
                  // Re-enable the submit button after the success
                  if (submitButton) {
                    submitButton.setEnabled(true);
                  }

                  // Unlock the submit button after the network request completes
                ...
                error: function (errorResponse) {
                  ...
                  // Re-enable the submit button after the error
                  if (submitButton) {
                    submitButton.setEnabled(true);
                  }
        ...
        onEditUserMaster: function () {
          ...
          if (!this.updateFragment) {
            this.updateFragment = sap.ui.xmlfragment(
              "com.usermasteraisp.usermasteraisp.fragments.updateUserMaster",
              this
            );
          ...
          var aSelectedEntity = sData.COMPANY_CODE
            ? sData.COMPANY_CODE.split(",")
            : [];
          var aSelectedRoles = sData.USER_ROLE
            ? sData.USER_ROLE.split(",").map((role) => role.trim())
            : [];
          sap.ui.getCore().byId("idUserIdEdit").setValue(sData.USER_ID); // Set User ID
          sap.ui
            .getCore()
            .byId("idEditUserRole")
            .setSelectedKeys(aSelectedRoles); // Set User Roles
          sap.ui.getCore().byId("idUserEditName").setValue(sData.USER_NAME); // Set User Name
          sap.ui
            .getCore()
            .byId("idCompanyCodeEdit")
            .setSelectedKeys(aSelectedEntity);
        ...
        onDeleteUserMaster: function () {
          if (!this.insertCommentFrag) {
            this.insertCommentFrag = sap.ui.xmlfragment(
              "com.usermasteraisp.usermasteraisp.fragments.insertComment",
              this
            );
        ...
        onDeleteConfirm: function (oEvent) {
          ...
          let currStatus = aSelectedItems[0]
            .getBindingContext()
            .getObject().STATUS;
        ...
        onShowMoreRoles: function (oEvent) {
          ...
          if (this.remainingRoles && this.remainingRoles.length > 0) {
            let fullText =
              oText.getText() + ", " + this.remainingRoles.join(", ");
