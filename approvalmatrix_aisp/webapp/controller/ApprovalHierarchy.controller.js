sap.ui.define(
  ...
  (Controller, MessageBox) => {
    ...
    return Controller.extend(
      "com.approvalmatrix.approvalmatrixaisp.controller.ApprovalHierarchy",
      {
        onInit() {
          this.editableData = false;
          this.getOwnerComponent()
            .getRouter()
            .getRoute("RouteHierarchyid")
            .attachPatternMatched(this._onRouteMatched, this);
          this.getOwnerComponent()
            .getRouter()
            .getRoute("RouteHierarchu")
        ...
        _onRouteMatchedwithoutid: function () {
          ...
          this.byId("entityFilter")
            .getInnerControls()[0]
            .setValue("")
            .setEditable(true);
          this.byId("idCreateFooterButton").setEnabled(true);
          this.byId("idObjTitle").setTitle("New Company");
              ...
              success: function (res) {
                ...
                dataArr.forEach((item) => {
                  oData.jsonObjects.push({
                    ...
                    sendBackCheckboxVisible:
                      item.APPR_TYPE == "R1" ? true : false,
        ...
        onPressCreateMassApprovalInputs: function () {
          debugger;
          var sCompanyCode = this.byId("entityFilter")
            .getInnerControls()[0]
            .getValue();
          var sApprovalType = this.byId("idCompoBoxReq")
            .getValue()
            .split("-")[0]
            ?.trim();
          ...
          var approvalType =
            sApprovalType !== "" ? sApprovalType : sApprovalTypeText;
          var oModel = this.getOwnerComponent().getModel("hierarchydata");
          var oData = oModel.getData();

          if (!sCompanyCode || !approvalType) {
            MessageBox.error(
              "Please select a valid Approval Type and Company Code first."
            );
            return;
          }
          ...
          if (approvalType === "Request" || approvalType === "R0") {
          ...
          oData.jsonObjects.push({
            ...
            editable: true,
            mdmBtn: false,
            isLocal: true, // Add flag to indicate locally added row
          });
          oModel.setData(oData);
        },
        ...
        onValueHelpRequested: function (oEvent) {
          ...
          if (!sSelectedRole || !sCompanyCode) {
            MessageBox.error(
              "Please select a valid Role and Company Code first."
            );
            return;
          }
          var aFilters = [
                ...
                oDialog.getTableAsync().then(
                  function (oTable) {
                    ...
                    if (oTable.bindItems) {
                      ...
                      oTable.addColumn(
                        new sap.m.Column({
                          header: new sap.m.Label({ text: "User ID" }),
                        })
                      );
                      oTable.addColumn(
                        new sap.m.Column({
        ...
        onValueHelpOkPress: function (oEvent) {
          ...
          var sPath = this._inputFieldRole
            .getBindingContext("hierarchydata")
            .getPath();
          if (aTokens.length > 0) {
            ...
            this.getView()
              .getModel("hierarchydata")
              .setProperty(sPath + "/role", sSelectedUser);

            // clear the second input field
            var sUserValue = this.getView()
              .getModel("hierarchydata")
              .getProperty(sPath + "/user"); // to get inputfiled
            if (sUserValue) {
              // Clear user from model
              this.getView()
                .getModel("hierarchydata")
                .setProperty(sPath + "/user", "");
        ...
        onValueHelpOkPressRoles: function (oEvent) {
          ...
          var sPath = this._inputFieldUser
            .getBindingContext("hierarchydata")
            .getPath();
          var aTokens = oEvent.getParameter("tokens");
          if (aTokens.length > 0) {
            ...
            this.getOwnerComponent()
              .getModel("hierarchydata")
              .setProperty(sPath + "/user", sSelectedRole);
          }
          this._oValueHelpDialog.close();
        },
        ...
        onFooterCreatePress: function () {
          ...
          if (!approvalType || !sCompanyCode) {
            MessageBox.error(
              "Please select a valid Approval Type and Company Code."
            );
            return;
          }

          // Get Approval Data from Table Model
          var aApprovalData = this.getOwnerComponent()
            .getModel("hierarchydata")
            .getProperty("/jsonObjects");

          // **Validation: Check if aApprovalData is empty or if any user or role is missing**
          var bInvalidEntry =
            !aApprovalData ||
            aApprovalData.length === 0 ||
            aApprovalData.some(function (item) {
              // Validate user (USER_ID) - must contain at least one alphabet
              if (
                !item.user ||
                !/[a-zA-Z]/.test(item.user) || // Ensure at least one letter
                !/^[a-zA-Z0-9!@#$%^&*()_+={}\[\]:;"'<>,.?/-]+$/.test(item.user) // Ensure no special chars only
              ) {
                // If user is empty, doesn't contain at least one letter, or contains only special characters
                return true;
              }

              // Validate role (USER_ROLE) - must contain at least one alphabet
              if (
                !item.role ||
                !/[a-zA-Z]/.test(item.role) || // Ensure at least one letter
                !/^[a-zA-Z0-9!@#$%^&*()_+={}\[\]:;"'<>,.?/-]+$/.test(item.role) // Ensure no special chars only
              ) {
                // If role is empty, doesn't contain at least one letter, or contains only special characters
                return true;
              }
          ...
          if (bInvalidEntry) {
            MessageBox.error(
              "Please ensure that both Role and User fields are valid (must contain at least one alphabet and valid characters)."
            );
            return;
          }
        ...
        onPressDeleteSelectedRows: function (oEvent) {
          ...
          let oData = oModel.getData(); // Get JSON model data
          ...
          aSelectedContexts.forEach((oContext) => {
            ...
            let iIndex = oData.jsonObjects.indexOf(oSelectedData); // Get index of the selected row

            if (
              oSelectedData.isLocal ||
              !oSelectedData.user ||
              !oSelectedData.role
            ) {
              // Case 1: Locally added row (isLocal: true or empty user/role)
              oData.jsonObjects.splice(iIndex, 1); // Remove the row from JSON model
              oModel.setData(oData); // Update the JSON model
              // sap.m.MessageToast.show(
              //   "Locally added row deleted successfully."
              // );
            } else {
              // Case 2: Persisted row, make network call
              let sCompanyCode = this.byId("entityFilter")
                .getInnerControls()[0]
                .getValue();
              let sApprovalType = this.byId("idCompoBoxReq")
                .getValue()
                .split("-")[0]
                ?.trim();
              ...
              oDataModel.create("/DeleteApprovalHierarchy", oPayload, {
                success: function (res) {
                  ...
                  oModel.setData(oData); // Update the JSON model
                  let successMessage =
                    res.DeleteApprovalHierarchy || "User Deleted Successfully!";
        ...
        //   oTable.setBusy(true);
        //   if (
        //     oSelectedData.user.length === 0 ||
        //     oSelectedData.role.length === 0
        //   ) {
        //     oTable.removeSelections();
        ...
        //         MessageBox.success(
        //           res.DeleteApprovalHierarchy ?? successMessage
        //         );
        //         oModel.updateBindings(true);
        //         oTable.removeSelections();
        //         let oBinding = oTable.getBinding("items");
